...
- `AgentResumeReq.launch_cmd_override` carries the Claude resume command (Claude only, client-stored preference). The client builds it from the unified `claude_command` profile as `<profile> --resume {session_id}`; the host substitutes `{session_id}` (alias `{quoted}`) with the **shell-quoted** session id and runs that command. The host always owns shell-quoting, so clients never assemble the final shell command. When the override is absent or lacks the token, the host falls back to the per-agent default resume command. Other agent kinds do not set this field today. The field is additive: same-version peers round-trip; new client → old host degrades gracefully (override ignored); old client → new host is unsupported during the upgrade window, consistent with the rest of the postcard schema.
- `AgentInstalledList` returns all supported terminal agent slugs with host-owned `launch_cmd` values for available CLIs. Clients create agents through `TermCreate` with that launch command. For Claude, the client substitutes its `claude_command` profile (default `ccs glm-lite --dangerously-skip-permissions`) for the host-resolved `claude` binary, so a new Claude terminal runs the profile directly; Claude also appears in the installed list when the host has no `claude` binary because the profile typically wraps a different CLI (e.g. `ccs`).
