...
## Using Local Host Build

Replace the globally installed `zedra` (from `zedra.dev`) with your local build so all projects on your machine use it.
...
Automatically picks up new builds after `cargo build`:

```bash
ln -s $(pwd)/target/debug/zedra ~/.local/bin/zedra
```

### Option B: Copy Release Binary

Smaller binary, but must re-copy after each build:
...
After either option, `zedra start` in any directory uses your local build.
